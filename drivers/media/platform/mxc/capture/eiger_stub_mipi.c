...
static const struct media_entity_operations eiger_stub_entity_ops = {
			 ...
			 const struct i2c_device_id *id)
{
	...
	v4l2_i2c_subdev_init(sd, client, &eiger_stub_subdev_ops);

	sd->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	//sd->internal_ops = &ar0144_subdev_internal_ops;
	sd->entity.function = MEDIA_ENT_F_CAM_SENSOR;
	sd->entity.ops = &eiger_stub_entity_ops;

	sensor->pad.flags = MEDIA_PAD_FL_SOURCE;

	retval = media_entity_pads_init(&sd->entity, 1, &sensor->pad);
	if (retval)
		goto out_media;
