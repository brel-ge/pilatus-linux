...
struct panel_info {
	unsigned short tp_max_x;
	unsigned short tp_max_y;
	unsigned char x_ch;
	unsigned char y_ch;
	unsigned char max_tp;
	unsigned char max_btn;
...
static bool ili210x_touchdata_to_coords(const u8 *touchdata,
					unsigned int finger, unsigned int *x,
...
static int ili251x_read_reg_common(struct i2c_client *client, u8 reg, void *buf,
...
static int ili251x_read_touch_data(struct i2c_client *client, u8 *data)
{
	...
	error = ili251x_read_reg_common(client, REG_TOUCHDATA, data,
					ILI251X_DATA_SIZE1, 0);
	if (!error && data[0] == 2) {
...
static int ili251x_firmware_update_resolution(struct device *dev)
{
	...
	dev_dbg(dev, "resx %u, resy %u", resx, resy);
...
static ssize_t ili251x_firmware_update_firmware_version(struct device *dev)
{
	...
	error = priv->chip->read_reg(client, REG_FIRMWARE_VERSION, &fw,
				     sizeof(fw));
	if (!error)
...
static ssize_t ili251x_firmware_update_kernel_version(struct device *dev)
{
	...
	/* Get kernel version */
	error = priv->chip->read_reg(client, REG_KERNEL_VERSION, &kv,
				     sizeof(kv));
	if (!error)
...
static ssize_t ili251x_firmware_update_protocol_version(struct device *dev)
{
	...
	error = priv->chip->read_reg(client, REG_PROTOCOL_VERSION, &pv,
				     sizeof(pv));
	if (!error)
...
static struct attribute *ili210x_attributes[] = {
	&dev_attr_calibrate.attr,      &dev_attr_firmware_version.attr,
	&dev_attr_kernel_version.attr, &dev_attr_protocol_version.attr,
	&dev_attr_mode.attr,	       NULL,
};
